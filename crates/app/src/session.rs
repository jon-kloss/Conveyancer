...
use serde::Serialize;
use solver::model::{
    EdgeSpec, FactorySnapshot, GroupSpec, InputPortSpec, NodeRef, OutputPortSpec, RecipeSpec, SolveResult, T0Edit,
    TargetCeiling,
};
...
impl Session {
    ...
    pub fn open(plan_path: impl AsRef<Path>, docs_json: Option<Vec<u8>>, game_build: &str) -> Result<Self, SessionError> {
    ...
    fn with_file(file: PlanFile, docs_json: Option<Vec<u8>>, game_build: &str) -> Result<Self, SessionError> {
        ...
        Ok(Self { state, undo, file, gamedata: gd, world, slow_solves: BTreeMap::new() })
    }
    ...
    pub fn edit(&mut self, cmds: Vec<Command>) -> Result<EditResponse, SessionError> {
        ...
        self.file.commit(&entry, &self.state.meta, self.undo.entries().len())?;
    ...
    pub fn undo(&mut self) -> Result<Option<EditResponse>, SessionError> {
        let Some(batch) = self.undo.undo(&mut self.state) else { return Ok(None) };
        self.file.checkpoint(&batch, &self.state.meta, self.applied_count())?;
        Ok(Some(self.nav_response(batch)))
    }

    pub fn redo(&mut self) -> Result<Option<EditResponse>, SessionError> {
        let Some(batch) = self.undo.redo(&mut self.state) else { return Ok(None) };
        self.file.checkpoint(&batch, &self.state.meta, self.applied_count())?;
        Ok(Some(self.nav_response(batch)))
    }
        ...
        for cmd in cmds {
            match cmd {
                Command::SetPortRate { id, rate } => {
                    return T0Edit::SetTarget { port: id.clone(), rate: *rate }
                }
                Command::SetGroupClock { id, clock } => {
                    return T0Edit::SetClock { group: id.clone(), clock: *clock }
                }
                _ => {}
            }
        }
    ...
    fn touched_factories(&self, cmds: &[Command]) -> Vec<Id> {
        ...
        for cmd in cmds {
            match cmd {
                ...
                | Command::DeleteGroup { id } => push(self.state.groups.get(id).map(|g| g.factory.clone())),
                Command::SetPortRate { id, .. }
                | Command::SetPortCeiling { id, .. }
                | Command::DeletePort { id } => push(self.state.ports.get(id).map(|p| p.factory.clone())),
                Command::SetEdgeTier { id, .. } | Command::DeleteEdge { id } => {
                    push(self.state.edges.get(id).map(|e| e.factory.clone()))
                }
                Command::ReleaseNode { id } => push(self.state.node_claims.get(id).map(|c| c.factory.clone())),
                _ => {}
            }
        }
        out
    ...
    pub fn snapshot(&self, fid: &Id) -> Option<FactorySnapshot> {
        ...
        for gid in &factory.groups {
            ...
            let power = self.gamedata.machines.get(&g.machine).map(|m| m.power_mw).unwrap_or(0.0);
        ...
        let edges = self
            ...
            .collect();
        Some(FactorySnapshot { groups, edges, inputs, outputs })
    }

    /// Extraction ceiling for a node claim, from gamedata (items/min).
    pub fn claim_rate(&self, claim: &NodeClaim) -> f64 {
        let Some(node) = self.world.nodes.iter().find(|n| n.id == claim.node) else { return 0.0 };
        let Some(machine) = self.gamedata.machines.get(&claim.extractor) else { return 0.0 };
    ...
    fn solve_factory_into_tx(&mut self, fid: &Id, trigger: &T0Edit, tx: &mut Transaction) {
        let Some(snapshot) = self.snapshot(fid) else { return };
        if snapshot.groups.is_empty() {
            return;
        }
        let trigger = self.trigger_for_factory(fid, &snapshot, trigger);
        let Ok(result) = solver::t1::solve(&snapshot, &trigger) else { return };
    ...
    fn trigger_for_factory(&self, _fid: &Id, snapshot: &FactorySnapshot, trigger: &T0Edit) -> T0Edit {
        match trigger {
            T0Edit::SetTarget { port, rate } if snapshot.outputs.iter().any(|p| &p.id == port) => {
                T0Edit::SetTarget { port: port.clone(), rate: *rate }
            }
            T0Edit::SetClock { group, clock } if snapshot.groups.iter().any(|g| &g.id == group) => {
                T0Edit::SetClock { group: group.clone(), clock: *clock }
            }
            _ => {
                if snapshot.outputs.len() == 1 {
                    T0Edit::SetTarget { port: snapshot.outputs[0].id.clone(), rate: snapshot.outputs[0].rate }
                } else {
                    T0Edit::Recompute
                }
            }
    ...
    pub fn solve_all_readonly(&mut self) -> Derived {
        ...
        for fid in fids {
            ...
            match solver::t1::solve(&snapshot, &trigger) {
                Ok(r) => {
                    ...
                    derived.factories.insert(fid, to_derived(&r, solve_on_release));
                }
                Err(e) => {
                    derived.factories.insert(
                        fid,
        ...
        for (node, claims) in by_node {
            derived.nodes.insert(node, DerivedNode { claims, conflict: claims > 1 });
        }
        derived
    }
}
...
fn to_derived(r: &SolveResult, solve_on_release: bool) -> DerivedFactory {
    DerivedFactory {
        ...
        edges: r
            .edges
            .iter()
            .map(|(id, e)| (id.clone(), DerivedEdge { flow: e.flow, saturation: e.saturation }))
            .collect(),
        ports: r.ports.clone(),
