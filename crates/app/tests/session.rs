...
fn connect(s: &mut Session, fid: &str, from: EdgeEnd, to: EdgeEnd, item: &str, tier: u8) -> Id {
    let r = s
        .edit(vec![Command::AddEdge { factory: fid.into(), from, to, item: item.into(), tier }])
        .unwrap();
    r.created[0].clone()
}
...
fn build_modular_frame_factory(s: &mut Session) -> (Id, Id, Id) {
    let r = s
        .edit(vec![Command::CreateFactory {
            ...
            position: MapPos { x: -1400.0, y: 2400.0 },
    ...
    let smelt = add_group(s, &fid, "Build_SmelterMk1_C", "Recipe_IngotIron_C", gp(200.0, 200.0));
    let rods = add_group(s, &fid, "Build_ConstructorMk1_C", "Recipe_IronRod_C", gp(420.0, 80.0));
    let plates = add_group(s, &fid, "Build_ConstructorMk1_C", "Recipe_IronPlate_C", gp(420.0, 320.0));
    let screws = add_group(s, &fid, "Build_ConstructorMk1_C", "Recipe_Screw_C", gp(640.0, 80.0));
    let rip = add_group(s, &fid, "Build_AssemblerMk1_C", "Recipe_IronPlateReinforced_C", gp(860.0, 260.0));
    let mf = add_group(s, &fid, "Build_AssemblerMk1_C", "Recipe_ModularFrame_C", gp(1100.0, 160.0));

    let g = EdgeEnd::Group;
    connect(s, &fid, EdgeEnd::Port(in_port.clone()), g(smelt.clone()), "Desc_OreIron_C", 3);
    connect(s, &fid, g(smelt.clone()), g(rods.clone()), "Desc_IronIngot_C", 3);
    connect(s, &fid, g(smelt.clone()), g(plates.clone()), "Desc_IronIngot_C", 3);
    connect(s, &fid, g(rods.clone()), g(screws.clone()), "Desc_IronRod_C", 2);
    connect(s, &fid, g(rods.clone()), g(mf.clone()), "Desc_IronRod_C", 2);
    connect(s, &fid, g(plates.clone()), g(rip.clone()), "Desc_IronPlate_C", 2);
    connect(s, &fid, g(screws.clone()), g(rip.clone()), "Desc_IronScrew_C", 2);
    connect(s, &fid, g(rip.clone()), g(mf.clone()), "Desc_IronPlateReinforced_C", 1);
    connect(s, &fid, g(mf.clone()), EdgeEnd::Port(out_port.clone()), "Desc_ModularFrame_C", 1);
...
fn exit_criterion_flow() {
    ...
    let (fid, out_port, smelt);
    {
        ...
        let r = s.edit(vec![Command::SetPortRate { id: out_port.clone(), rate: 2.0 }]).unwrap();
        ...
        let in_port_rate: f64 = df
            ...
            .map(|(_, r)| *r)
            .sum();
        assert!((in_port_rate - 48.0).abs() < 1e-4, "ore at 24T: {in_port_rate}");
        ...
        assert!((ceiling.max_rate - 5.0).abs() < 1e-4, "max {}", ceiling.max_rate);
        ...
        assert_eq!(s.state.groups[&smelt].count, 1, "solve write-back undone with the edit");
        assert!(r.can_redo);
...
fn infeasible_target_clamps_and_names_constraint() {
    ...
    let r = s.edit(vec![Command::SetPortRate { id: out_port.clone(), rate: 8.0 }]).unwrap();
    let df = &r.derived.factories[&fid];
...
fn failed_multi_command_edit_rolls_back() {
    ...
    let err = s.edit(vec![
        Command::RenameFactory { id: fid.clone(), name: "Y".into() },
        Command::RenameFactory { id: "missing".into(), name: "Z".into() },
    ]);
    assert!(err.is_err());
