...
pub fn write(conn: &Connection, gd: &GameData) -> Result<(), DbError> {
    ...
    conn.execute("INSERT OR REPLACE INTO meta (key, value) VALUES ('game_build', ?1)", [&gd.build_version])?;
    ...
    for (class, item) in &gd.items {
        conn.execute("INSERT INTO items (class, json) VALUES (?1, ?2)", (class, serde_json::to_string(item)?))?;
    }
    for (class, r) in &gd.recipes {
        conn.execute("INSERT INTO recipes (class, json) VALUES (?1, ?2)", (class, serde_json::to_string(r)?))?;
    }
    for (class, m) in &gd.machines {
        conn.execute("INSERT INTO machines (class, json) VALUES (?1, ?2)", (class, serde_json::to_string(m)?))?;
    }
    for (class, b) in &gd.belts {
        conn.execute("INSERT INTO belts (class, json) VALUES (?1, ?2)", (class, serde_json::to_string(b)?))?;
    }
...
pub fn read(conn: &Connection) -> Result<GameData, DbError> {
    let build_version: String = conn
        .query_row("SELECT value FROM meta WHERE key = 'game_build'", [], |r| r.get(0))
        .unwrap_or_default();
    let mut gd = GameData { build_version, ..Default::default() };
    ...
    for (class, json) in load("recipes")? {
        gd.recipes.insert(class, serde_json::from_str::<Recipe>(&json)?);
    }
    for (class, json) in load("machines")? {
        gd.machines.insert(class, serde_json::from_str::<Machine>(&json)?);
    }
...
pub fn build_matches(conn: &Connection, build: &str) -> bool {
    conn.query_row("SELECT value FROM meta WHERE key = 'game_build'", [], |r| r.get::<_, String>(0))
...
pub fn manufacturer_for(gd: &GameData, recipe: &Recipe) -> Option<String> {
    recipe.produced_in.iter().find(|c| {
        gd.machines.get(*c).map(|m| matches!(m.kind, MachineKind::Manufacturer)).unwrap_or(false)
    }).cloned()
}
...
mod tests {
    ...
    fn roundtrip_through_sqlite() {
        ...
        assert_eq!(manufacturer_for(&back, mf).as_deref(), Some("Build_AssemblerMk1_C"));
    }
}
