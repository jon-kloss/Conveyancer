...
pub enum Command {
    CreateFactory { name: String, position: MapPos, region: String },
    RenameFactory { id: Id, name: String },
    MoveFactoryPin { id: Id, position: MapPos },
    DeleteFactory { id: Id },
    AddGroup { factory: Id, machine: String, recipe: String, count: u32, clock: f64, graph_pos: GraphPos },
    SetGroupRecipe { id: Id, machine: String, recipe: String },
    SetGroupCount { id: Id, count: u32 },
    SetGroupClock { id: Id, clock: f64 },
    MoveGroupCard { id: Id, graph_pos: GraphPos },
    DeleteGroup { id: Id },
    AddPort { factory: Id, direction: PortDirection, item: String, rate: f64, rate_ceiling: Option<f64>, graph_pos: GraphPos },
    SetPortRate { id: Id, rate: f64 },
    SetPortCeiling { id: Id, rate_ceiling: Option<f64> },
    MovePortCard { id: Id, graph_pos: GraphPos },
    DeletePort { id: Id },
    AddEdge { factory: Id, from: EdgeEnd, to: EdgeEnd, item: String, tier: u8 },
    SetEdgeTier { id: Id, tier: u8 },
    DeleteEdge { id: Id },
    ClaimNode { factory: Id, node: String, extractor: String, clock: f64 },
    ReleaseNode { id: Id },
...
fn require_planned(status: Status, id: &Id, action: &str) -> Result<(), DomainError> {
    ...
    if status == Status::Built {
        return Err(DomainError::BuiltImmutable { id: id.clone(), action: action.into() });
    }
    Ok(())
}
...
fn clamp_clock(clock: f64) -> Result<f64, DomainError> {
    if !(0.01..=2.5).contains(&clock) {
        return Err(DomainError::Invalid { message: format!("clock {clock} outside 1%–250%") });
    }
    Ok(clock)
}
...
fn valid_tier(tier: u8) -> Result<u8, DomainError> {
    if !(1..=6).contains(&tier) {
        return Err(DomainError::Invalid { message: format!("belt tier {tier} outside Mk.1–Mk.6") });
    }
    Ok(tier)
}
...
pub fn apply(state: &mut PlanState, cmd: &Command) -> Result<Transaction, DomainError> {
    ...
    match cmd {
        Command::CreateFactory { name, position, region } => {
            let f = Factory {
                id: new_id(),
                name: name.clone(),
                position: *position,
        ...
        Command::RenameFactory { id, name } => {
            let mut f = state.factories.get(id).cloned().ok_or(DomainError::NotFound { id: id.clone() })?;
            f.name = name.clone();
            tx.record(state.upsert(Entity::Factory(f)));
        }
        ...
        Command::DeleteFactory { id } => {
            let f = state.factories.get(id).cloned().ok_or(DomainError::NotFound { id: id.clone() })?;
            require_planned(f.status, id, "delete")?;
            // Cascade: groups, ports, edges, claims belonging to this factory.
            let group_ids: Vec<Id> = state.groups.values().filter(|g| &g.factory == id).map(|g| g.id.clone()).collect();
            let port_ids: Vec<Id> = state.ports.values().filter(|p| &p.factory == id).map(|p| p.id.clone()).collect();
            let edge_ids: Vec<Id> = state.edges.values().filter(|e| &e.factory == id).map(|e| e.id.clone()).collect();
            let claim_ids: Vec<Id> = state.node_claims.values().filter(|c| &c.factory == id).map(|c| c.id.clone()).collect();
            for eid in edge_ids {
                if let Some(ops) = state.remove(COLL_EDGES, &eid) { tx.record(ops); }
            }
            for gid in group_ids {
                if let Some(ops) = state.remove(COLL_GROUPS, &gid) { tx.record(ops); }
            }
            for pid in port_ids {
                if let Some(ops) = state.remove(COLL_PORTS, &pid) { tx.record(ops); }
            }
            for cid in claim_ids {
                if let Some(ops) = state.remove(COLL_NODE_CLAIMS, &cid) { tx.record(ops); }
            }
            if let Some(ops) = state.remove(COLL_FACTORIES, id) { tx.record(ops); }
        }
        Command::AddGroup { factory, machine, recipe, count, clock, graph_pos } => {
            let mut f = state.factories.get(factory).cloned().ok_or(DomainError::NotFound { id: factory.clone() })?;
            let g = MachineGroup {
                id: new_id(),
        ...
        Command::SetGroupRecipe { id, machine, recipe } => {
            let mut g = state.groups.get(id).cloned().ok_or(DomainError::NotFound { id: id.clone() })?;
        ...
        Command::DeleteGroup { id } => {
            let g = state.groups.get(id).cloned().ok_or(DomainError::NotFound { id: id.clone() })?;
            ...
            if let Some(ops) = state.remove(COLL_GROUPS, id) { tx.record(ops); }
        }
        Command::AddPort { factory, direction, item, rate, rate_ceiling, graph_pos } => {
            let mut f = state.factories.get(factory).cloned().ok_or(DomainError::NotFound { id: factory.clone() })?;
            let p = Port {
                id: new_id(),
                factory: factory.clone(),
        ...
        Command::SetPortRate { id, rate } => {
            let mut p = state.ports.get(id).cloned().ok_or(DomainError::NotFound { id: id.clone() })?;
            if *rate < 0.0 {
                return Err(DomainError::Invalid { message: "rate must be ≥ 0".into() });
            }
            p.rate = *rate;
            tx.record(state.upsert(Entity::Port(p)));
        }
        ...
        Command::DeletePort { id } => {
            let p = state.ports.get(id).cloned().ok_or(DomainError::NotFound { id: id.clone() })?;
            ...
            if let Some(ops) = state.remove(COLL_PORTS, id) { tx.record(ops); }
        }
        Command::AddEdge { factory, from, to, item, tier } => {
            state.factories.get(factory).ok_or(DomainError::NotFound { id: factory.clone() })?;
            let e = BeltEdge {
                id: new_id(),
                factory: factory.clone(),
                from: from.clone(),
        ...
        Command::SetEdgeTier { id, tier } => {
            let mut e = state.edges.get(id).cloned().ok_or(DomainError::NotFound { id: id.clone() })?;
            e.tier = valid_tier(*tier)?;
            tx.record(state.upsert(Entity::Edge(e)));
        }
        Command::DeleteEdge { id } => {
            let e = state.edges.get(id).cloned().ok_or(DomainError::NotFound { id: id.clone() })?;
            require_planned(e.status, id, "delete")?;
            if let Some(ops) = state.remove(COLL_EDGES, id) { tx.record(ops); }
        }
        Command::ClaimNode { factory, node, extractor, clock } => {
            let mut f = state.factories.get(factory).cloned().ok_or(DomainError::NotFound { id: factory.clone() })?;
        ...
        Command::ReleaseNode { id } => {
            let c = state.node_claims.get(id).cloned().ok_or(DomainError::NotFound { id: id.clone() })?;
            if let Some(mut f) = state.factories.get(&c.factory).cloned() {
                f.node_claims.retain(|cid| cid != id);
                tx.record(state.upsert(Entity::Factory(f)));
            }
            if let Some(ops) = state.remove(COLL_NODE_CLAIMS, id) { tx.record(ops); }
        }
        Command::RenamePlan { name } => {
            let old = state.meta.name.clone();
            state.meta.name = name.clone();
            tx.forward.push(PatchOp::Replace { path: "/meta/name".into(), value: serde_json::json!(name) });
            tx.inverse.push(PatchOp::Replace { path: "/meta/name".into(), value: serde_json::json!(old) });
        }
    }
    Ok(tx)
}
