...
fn recipe(id: &str, machine: &str, dur: f64, inputs: &[(&str, f64)], outputs: &[(&str, f64)], mw: f64) -> RecipeSpec {
    RecipeSpec {
        id: id.into(),
...
fn group(id: &str, r: RecipeSpec) -> GroupSpec {
    GroupSpec { id: id.into(), recipe: r, count: 1, clock: 1.0 }
}

fn edge(id: &str, from: NodeRef, to: NodeRef, item: &str, capacity: f64) -> EdgeSpec {
    EdgeSpec { id: id.into(), from, to, item: item.into(), capacity }
}
...
fn modular_frame_snapshot(target: f64, ore_ceiling: Option<f64>, screw_belt_cap: f64) -> FactorySnapshot {
    FactorySnapshot {
        groups: vec![
            group("smelt", recipe("Recipe_IngotIron_C", "smelter", 2.0, &[("ore", 1.0)], &[("ingot", 1.0)], 4.0)),
            group("rods", recipe("Recipe_IronRod_C", "constructor", 4.0, &[("ingot", 1.0)], &[("rod", 1.0)], 4.0)),
            group("screws", recipe("Recipe_Screw_C", "constructor", 6.0, &[("rod", 1.0)], &[("screw", 4.0)], 4.0)),
            group("plates", recipe("Recipe_IronPlate_C", "constructor", 6.0, &[("ingot", 3.0)], &[("plate", 2.0)], 4.0)),
            group("rip", recipe("Recipe_IronPlateReinforced_C", "assembler", 12.0, &[("plate", 6.0), ("screw", 12.0)], &[("rip", 1.0)], 15.0)),
            group("mf", recipe("Recipe_ModularFrame_C", "assembler", 60.0, &[("rip", 3.0), ("rod", 12.0)], &[("mf", 2.0)], 15.0)),
        ],
        edges: vec![
            edge("e-ore", NodeRef::Input("in-ore".into()), g("smelt"), "ore", 780.0),
            ...
            edge("e-screw-rip", g("screws"), g("rip"), "screw", screw_belt_cap),
            edge("e-rip-mf", g("rip"), g("mf"), "rip", 780.0),
            edge("e-mf-out", g("mf"), NodeRef::Output("out-mf".into()), "mf", 780.0),
        ],
        inputs: vec![InputPortSpec { id: "in-ore".into(), item: "ore".into(), ceiling: ore_ceiling }],
        outputs: vec![OutputPortSpec { id: "out-mf".into(), item: "mf".into(), rate: target }],
    }
}
...
fn assert_golden_rates(r: &SolveResult, t: f64) {
    ...
    assert_close(r.groups["screws"].out_rates["screw"], 18.0 * t, "screw rate");
...
fn t0_modular_frame_golden() {
    ...
    let r = solver::t0::solve(&snap, &T0Edit::SetTarget { port: "out-mf".into(), rate: 2.0 }).unwrap();
...
fn t1_modular_frame_golden() {
    let snap = modular_frame_snapshot(0.0, None, 780.0);
    let r = solver::t1::solve(&snap, &T0Edit::SetTarget { port: "out-mf".into(), rate: 2.0 }).unwrap();
    assert_golden_rates(&r, 2.0);
    assert!(!r.clamped);
}
...
fn t0_t1_parity_within_epsilon() {
    ...
    for target in [0.5, 1.0, 2.0, 3.0] {
        let edit = T0Edit::SetTarget { port: "out-mf".into(), rate: target };
        ...
        for (id, ga) in &a.groups {
            ...
            for (item, rate) in &ga.out_rates {
                assert!((rate - gb.out_rates[item]).abs() < 1e-4, "parity {id}/{item}: t0={rate} t1={}", gb.out_rates[item]);
            }
            assert_eq!(ga.count, gb.count, "count parity for {id}");
        }
        for (id, ea) in &a.edges {
            assert!((ea.flow - b.edges[id].flow).abs() < 1e-4, "edge parity {id}");
        }
    }
}
...
fn t0_hard_stops_at_belt_capacity_and_names_it() {
    ...
    let r = solver::t0::solve(&snap, &T0Edit::SetTarget { port: "out-mf".into(), rate: 5.0 }).unwrap();
    ...
    assert_close(r.edges["e-screw-rip"].saturation, 1.0, "screw belt saturation");
}
...
fn t1_hard_stops_at_input_ceiling_and_names_it() {
    ...
    let r = solver::t1::solve(&snap, &T0Edit::SetTarget { port: "out-mf".into(), rate: 3.0 }).unwrap();
    assert!(r.clamped);
    let ceiling = r.target_ceiling.clone().expect("ceiling must be reported");
...
fn mass_balance_property() {
    ...
    for i in 1..=20 {
        let t = i as f64 * 0.35;
        let r = solver::t0::solve(&snap, &T0Edit::SetTarget { port: "out-mf".into(), rate: t }).unwrap();
        for gspec in &snap.groups {
            ...
            for (item, _) in &gspec.recipe.inputs {
                let inflow: f64 = snap
                    ...
                    .sum();
                assert!((inflow - gr.in_rates[item]).abs() < 1e-6, "conservation at {}/{item}", gspec.id);
            }
        }
    }
}
...
fn clock_edit_rederives_count() {
    ...
    let r = solver::t0::solve(&snap, &T0Edit::SetClock { group: "smelt".into(), clock: 0.5 }).unwrap();
    assert_eq!(r.groups["smelt"].count, 4);
    assert_close(r.groups["smelt"].clock, 0.5, "explicit clock preserved");
    assert_close(r.groups["smelt"].out_rates["ingot"], 48.0, "rate unchanged by clock edit");
}
