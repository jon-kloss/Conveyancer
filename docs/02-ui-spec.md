...
> **Superseded for belt/route FLOW by the efficiency grammar (DECISIONS "Flow EFFICIENCY grammar" / "Efficiency grammar completion"):** amber = flowing at ≤50% utilization (UNDER-USED), green = >50% INCLUDING a full belt whose demand is met, red = solver-evidenced BOTTLENECK only — never a bare % threshold. Token hexes above are unchanged (meanings renamed in place). Power margins and node-ceiling meters deliberately keep congestion thresholds — grids brown out and nodes exhaust.
...
- **Gating (behavior):** local heuristics arm it (new deficit, conflict, deficit through a full route, power swing), 30s debounce, visible hourly call budget, per-rule mute on dismiss. Offline/no key: `AI OFFLINE` chip; local heuristic warnings keep flowing into the same feed.
...
- Advisor: event gate (deficit/conflict/deficit through a full route/power swing) → 30s debounce → budget check → call; provenance stored per card. Chat context scope: empire/factory/selection snapshots.
