// AUDIT area: import — drift-accept cascade correctness + save-only node
// synthesis, driven end-to-end through the dev bridge's import pipeline
// (/api/new_empire → /api/import/run → /api/proposal/accept → /api/hydrate).
// Every probe declares its EXPECTED (correct) result in the header BEFORE any
// assertion; where the current build is known to fail, the assertion still
// pins the CORRECT behavior — a failing probe is data for the mismatch
// protocol, never a reason to weaken the assert.
//
// Seeded entirely through the API against the dev bridge's default fixture
// catalog (Build_SmelterMk1_C / Recipe_IngotIron_C @ 30/min, Build_ConstructorMk1_C
// / Recipe_IronRod_C @ 15/min, Build_MinerMk1_C) and the bundled world-node
// catalog (all nodes within x∈[-2818,4066], y∈[-3013,3029] — a miner at
// 900000,900000 is ~1.27M m from the nearest, far past NODE_MATCH_M=250 m).
// These probes assert purely over /api/hydrate, so they need no open page.
...
interface DerivedGroup { outRates: Record<string, number> }
interface DerivedFactory { groups: Record<string, DerivedGroup>; ports: Record<string, number> }
interface Hydrate {
  plan: {
...
// Delete every factory in the plan (fresh-empire probes own the whole plan) so
// no imported ◆ layer, claim, or drift proposal leaks into later specs.
async function cleanup(request: APIRequestContext): Promise<void> {
  try {
    const h = await hydrate(request);
    const cmds = vals(h.plan.factories).map((f) => ({ type: "delete_factory", id: f.id }));
...
// ---------------------------------------------------------------------------
// PROBE 1 — Demolished-group drift accept cascades (no orphaned edges/ports, no
// phantom output).
//
...
// derived output does not list Desc_IronRod_C. (Current build FAILS: the ingot
// smelter→constructor edge, the constructor→rod-port edge, and the
// Desc_IronRod_C Out port all survive as orphans, and the derived factory still
// reports an IronRod export sourced from the deleted constructor.)
// ---------------------------------------------------------------------------
test("demolished-group drift accept cascades — no orphaned edges/ports/output", async ({ request }) => {
  ...
  try {
    ...
    // OUT port keyed for a surviving rod port. (Under the bug the orphan rod
    // port still keys a derived export sourced from the deleted constructor.)
...
// Desc_IronIngot_C Out port rate is 180/min (doubled), and the derived factory
// output for Desc_IronIngot_C is 180/min. (Current build FAILS: group.count
// becomes 6 but the Out port rate stays 90/min and the demand-driven solve caps
// derived output at 90/min, so the expanded factory under-reports by 90/min.)
// ---------------------------------------------------------------------------
...
test("import synthesizes a save-only node claim + override off the catalog", async ({ request }) => {
  ...
  try {
    ...
    // miner's own position (≈ 900000,900000) so the node resolves into the map.
    ...
    expect(overrides[0].pos!.x).toBeCloseTo(900000, 1);
