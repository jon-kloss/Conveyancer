// AUDIT area: make — MAKE FROM RESOURCES planner math vs the fixture catalog.
...
//
// KNOWN MISMATCH (documented, do NOT weaken): under current code the capacity
// guard sizes against the FULL fresh raw demand (30 ingot) ignoring the reuse
// redirect, so mfr-warn IS visible and mfr-build IS disabled — the build cannot
// proceed. This probe is EXPECTED to fail at the "no warn / enabled" assertions.
// ---------------------------------------------------------------------------
...
// per*count*clock = 30 >= 20) with clock unchanged at 1.0; exactly one
// Recipe_Screw_C group (count 2); port-out Desc_IronScrew_C shows 80/min.
...
test("reuse scales up the rod machine count when capacity is short", async ({ page, request }) => {
  ...
  try {
    ...
    expect(rodG.clock).toBeCloseTo(1.0, 3); // clock left alone; count did the work
    expect(countRecipe(h, f, "Recipe_Screw_C")).toBe(1);
    expect(groupsOf(h, f, "Recipe_Screw_C")[0].count).toBe(2);
