// AUDIT area: wizard — supply-chain wizard → proposal review surface
...
//
// EXPECTED (honest): N === M — both should be the clock-scaled solved draw of
// the four @62% machines. The two power figures the review shows together must
// agree. (KNOWN MISMATCH, documented, do NOT weaken: current code computes the
// CREATE impact N as recipe_power × count WITHOUT the clock factor, while the
// footer M is the real solver draw scaled by clock^1.321928; for @62% stages
// N > M, so this probe is EXPECTED to FAIL — pinning that the two figures shown
// side by side disagree.)
// ---------------------------------------------------------------------------
