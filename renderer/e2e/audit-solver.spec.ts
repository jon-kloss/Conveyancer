// Audit #123 acceptance (promoted from the audit probe suite): the T0
// drag-preview must not collapse an independent chain when a SIBLING target is
// infeasible; the output-target hard-stop clamps at the true input ceiling and
// names it; and an un-wired driven generator holds its fuel-limited nameplate.
//
...
// dev bridge's default fixture catalog:
...
//
// EXPECTED: target-value shows ~20/min (out-x's own dragged rate) and out-x's
// constructor node (constructor A) shows ~20/min — NOT 0. The two chains are
// independent, so out-y's infeasibility against in-b's ceiling must not touch
// out-x's projected preview.
//
// (Fixed by audit #123: the T0 ceiling analysis judges feasibility RELATIVE to
// the base state, so a pre-existing sibling violation no longer zeroes the
// edited chain. This test is the permanent regression guard.)
// ---------------------------------------------------------------------------
