...
export default function ReceiveFromFactory({
  ...
  // the chosen cargo kind. A mixed pull wires each item on its own medium.
  ...
  // Which media the current selection needs — a pipe control shows for fluids,
  // the belt/rail/truck/drone picker for solids (both, on a mixed pull).
      ...
      <div className="send-modal" onClick={(e) => e.stopPropagation()}>
        ...
        {sources.length === 0 ? (
          ...
          <>
            ...
            {/* Fluids ride a pipe — show its tier + capacity, like the map's route
                popover. On a mixed pull the belt controls above cover the solids. */}
            {hasFluid && (
              <label className="send-row">
